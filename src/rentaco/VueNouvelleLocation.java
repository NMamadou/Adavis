...
public class VueNouvelleLocation extends JPanel implements ActionListener, DocumentListener {
	...
	private List<String> nomVehicule = new ArrayList<String>();
	...
	public VueNouvelleLocation(ModeleLocations modele, Controleur controleur) {
		...
		for(Client client : this.modele.getClients()){
			numClient.add(new Integer(client.getNumero()));
		...
		for(Vehicule vehicule : this.modele.getVehiculesDisponibles()){
			nomVehicule.add(new String(vehicule.getImmatriculation()));
	...
	public void actionPerformed(ActionEvent evt) {
		...
		int numeroClient = numClient.get(indiceClient).intValue();
		int indiceVehicule = cbVehicules.getSelectedIndex();
		String immVehicule = nomVehicule.get(indiceVehicule);
		if(sourceEvt == this.bEnregistrer){
			...
			//this.controleur.enregistrerLocation( ,  , tfDateDepart.getText());
		}
